        ...
        <el-table size="mini" ref="multipleTable" border style="width:100%" height="680px" :data="data" @selection-change="handleSelectionChange">
            ...
            <el-table-column label="资源更新" align="center" width="100">
                <template slot-scope="scope">
					...
					<br/>
					<el-upload :action="uploadMVUrl(scope.row.id)" :before-upload="beforeMVUpload" 
                        :on-success="handleSongSuccess">
                        <el-button size="mini">更新MV</el-button>
                    </el-upload>
                </template>
            </el-table-column>
...
export default {
    ...
    methods:{
        ...
        beforeMVUpload(file){
            var testMsg = file.name.substring(file.name.lastIndexOf('.') + 1);
            if(testMsg!='mp4' && testMsg!='avi' && testMsg!='wmv' && testMsg!='rmvb' && testMsg!='rm' && testMsg!='flv'){
                this.$message({
                    message: '上传文件只能是mp4，avi，wmv，rmvb，rm或flv格式',
                    type: 'error'
                });
                return false;
            }
		...
		//更新歌曲MV url
        uploadMVUrl(id){
            return `${this.$store.state.HOST}/song/updateMVUrl?id=${id}`
        },
        //切换播放歌曲
        setSongUrl(url,name) {
            this.toggle = name;
